...
from app import get_traffic_plan
import requests
...
def fetch_traffic_plan_from_app():
    """Fetch traffic plan dynamically from Flask app"""
    try:
        response = requests.get("http://127.0.0.1:5000/traffic", timeout=2)
        data = response.json()
        phases, green_times, counts_by_road = [], [], {}

        for ph in data.get("phases", []):
            green_roads = [road for road, state in ph["lights"].items() if state == "GREEN"]
            phases.append(green_roads)
            green_times.append(ph.get("time", 6.0))
        ...
        if not phases:
            ...
            # Counts for initial car spawn
            for road in ["north", "south", "east", "west"]:
                counts_by_road[road] = 0
                for ph in data.get("phases", []):
                    if road in ph["lights"]:
                        counts_by_road[road] += 1

        return phases, green_times, counts_by_road

    except Exception as e:
        print(f"Error fetching traffic plan from Flask: {e}")
        # fallback
        return [["north", "south"], ["east", "west"]], [15.0, 15.0], {"north":5,"south":5,"east":5,"west":5}
...
def main():
    ...
    try:
        ...
        phases, green_times, counts_by_road = fetch_traffic_plan_from_app()
    except Exception as e:
